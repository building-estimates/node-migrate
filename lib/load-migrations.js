  ...
  store.load(function (err, state) {
    ...
    fs.readdir(migrationsDirectory, function (err, files) {
      ...
      let migrations = files.map(function (file) {
        ...
        let mod
        try {
          const BaseClass = require(path.join(migrationsDirectory, file)).default
          mod = new BaseClass()
        } catch (e) {
          return fn(e)
        }

        const migration = new Migration(file, mod.up.bind(mod), mod.down.bind(mod), mod.description)
